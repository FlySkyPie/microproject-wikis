...
Here's a screenshot of a default Decoda window with the debugger stepping through the code (scaled down): ![Decoda workspace](Static/decoda_workspace.png)
...
Next you need to set up the executable that Decoda will run when debugging your files. Select menu Project -> Settings. A new dialog will open: ![Decoda debug settings](Static/decoda_debug_settings.png)
...
- Unfortunately for us Windows users, the Decoda project file uses Unix-lineends (CR only). This makes it problematic when checking the file into a version control system, since those usually expect windows (CRLF) lineends; I personally convert the lineends each time I edit the project file using [TED Notepad](http://jsimlo.sk/notepad/).