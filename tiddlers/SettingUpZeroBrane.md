...
Here's a screenshot of a default ZBS window with the debugger stepping through the code (scaled down): ![ZBS workspace](Static/zbs_workspace.png)
...
Since ZBS is a universal Lua IDE, you need to first set it up so that it is ready for Cuberite plugin development. For that, you need to download one file, [cuberite.lua](https://raw.githubusercontent.com/pkulchenko/ZeroBranePackage/master/cuberite.lua) from the [ZBS's plugin repository](https://github.com/pkulchenko/ZeroBranePackage). Place that file in the "packages" folder inside your ZBS's folder. Note that there are other useful plugins in the repository and you may want to have a look there later on to further customize your ZBS. To install them, simply save them into the same folder.
